...
export const SnackButton: React.FC<SnackButtonProps> = ({ 
  ...
  // Safely calculate total stock from options, ensuring we only add finite numbers
  const totalOptionsStock = hasOptions 
    ? item.options.reduce((acc, o) => {
        const s = typeof o === 'object' ? Number(o.stock) : 0;
        return acc + (Number.isFinite(s) ? s : 0);
      }, 0)
    : 0;

  const itemStockValue = Number(item.stock);
  const safeItemStock = Number.isFinite(itemStockValue) ? itemStockValue : 0;

  // Logic: if item has options, use the options sum, otherwise use item.stock
  const displayStock = hasOptions ? totalOptionsStock : safeItemStock;
  
  // Is out of stock if display stock minus what's in cart is <= 0
  const isOutOfStock = displayStock <= currentInCart;
  ...
  return (
    ...
    >
      <div className={`absolute top-2 right-2 px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-tighter ${displayStock < 5 ? 'bg-red-500 animate-pulse' : 'bg-black/40'}`}>
        {displayStock} left
      </div>
